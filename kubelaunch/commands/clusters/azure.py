...
def azure_commands(self):
    ...
    if lc['a_service_type']=="aks":
        commands['create']="az aks create --resource-group="+lc['a_resource_group'] +" --name="+lc['a_cluster_name']+" --agent-count="+str(lc['a_num_nodes'])+" --agent-vm-size="+lc['a_machine_type']+" --generate-ssh-keys --no-wait"
        commands['describe']="az aks list --resource-group="+lc['a_resource_group']
        commands['delete']="az aks delete --name="+lc['a_cluster_name']+" --resource-group="+lc['a_resource_group']+" --no-wait"
        commands['get_credentials']="az aks get-credentials --resource-group="+lc['a_resource_group']+" --name="+lc['a_cluster_name']
        commands['normal_size']="az aks scale  --agent-count "+str(lc['a_num_nodes']) +" --name="+lc['a_cluster_name']+" --resource-group="+lc['a_resource_group']
        commands['class_size']="az aks scale --agent-count "+ str(lc['a_num_nodes_class'])+" --name="+lc['a_cluster_name']+" --resource-group="+lc['a_resource_group']
    elif lc['a_service_type']=="acs":
        commands['create']="az acs create --orchestrator-type=kubernetes --resource-group="+lc['a_resource_group'] +" --name="+lc['a_cluster_name']+" --dns-prefix="+lc['a_dns_prefix']+" --agent-count="+str(lc['a_num_nodes'])+" --agent-vm-size="+lc['a_machine_type']+" --generate-ssh-keys --no-wait"
        commands['describe']="az acs list  --resource-group="+lc['a_resource_group']
        commands['delete']="az acs delete  --resource-group="+lc['a_resource_group']+" --name="+lc['a_cluster_name']+" --no-wait"
